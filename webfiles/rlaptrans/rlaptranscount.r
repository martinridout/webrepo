#=============================================================================================
rlaptranscount <- function(n, ltpdf, ..., tol=1e-7, x0=1, xinc=2, m=11, L=1, A=19, nburn=38)
#=============================================================================================
{
...
#    cat("kount0 = ",kount0,t,cdf,"\n")
    ...
    for (j in 1:n) {
        ...
        xrand[j] = t
        lower = t
#        cat("j,kount",j,kount[j],"\n")
    }

#cat(kount0, kount[1], mean(kount[2:n]), sd(kount[2:n]),"\n")
meankount = (kount0 + sum(kount))/n
    if (n > 1) {
       rsample <- sample(xrand) }
     else {
       rsample <- xrand} 
    list(x = rsample, count= meankount)
}